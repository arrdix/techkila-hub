...
export function FlatCard({ children }: { children: ReactNode }): JSX.Element {
    return <div className="flex-flex-col border px-4 rounded-xl">{children}</div>
}
...
const Body = forwardRef<HTMLDivElement, Body>(({ children, className, ...props }, ref) => {
    return (
        <>
            <div ref={ref} className={cn('flex py-3', className)} {...props}>
                {children}
            </div>
            <Separator />
        </>
...
const Footer = forwardRef<HTMLDivElement, Footer>(({ children, className, ...props }, ref) => {
    return (
        <div
            ref={ref}
            className={cn('flex justify-between items-center py-3', className)}
            {...props}
        >
            {children}
        </div>
